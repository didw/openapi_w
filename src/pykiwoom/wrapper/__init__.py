...
class KiwoomWrapper:
    ...
    def get_data_opc10001(self, code):
        self.kiwoom.data_opc10001 = [] * 5
        self.kiwoom.set_input_value("종목코드", code)
        self.kiwoom.set_input_value("시간단위", 1)
        self.kiwoom.comm_rq_data("해외선물옵션틱차트조회", "opc10001", 0, "0101")
        self.inquiry = 1
        while self.inquiry != 0:
            continue
        self.kiwoom.data_opc10001.index = self.kiwoom.data_opc10001.loc[:, '체결시간']
        return self.kiwoom.data_opc10001.loc[:, ['현재가', '거래량', '시가', '고가', '저가']]
