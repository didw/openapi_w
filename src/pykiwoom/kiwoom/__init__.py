...
class Kiwoom(QAxWidget):
    ...
    def on_receive_tr_data(self, screen_no, request_name, tr_code, unused1, recname_inquiry):
        """
        TR 수신 이벤트
        ...
        """
        print(recname_inquiry.split())
        record_name, inquiry, unused2 = recname_inquiry.split()
        ...
        if int(inquiry) == 1:
            self.inquiry = 1
        if request_name == "해외선물옵션틱차트조회":
            data = self.get_comm_data(tr_code, "해외선물옵션틱차트조회")
            print(data)
            if data is not None:
                data = list(map(lambda x: list(map(lambda y: y.replace('+','').replace('--','-'), x)), np.array(data)[:,0:7].tolist()))
                self.data_opc10001.extend(data)
    ...
    def get_global_future_item_list(self):
        func = 'GetGlobalFutureItemlist()'
        codes = self.dynamicCall(func)
        return codes.split(';')

    def get_global_option_code_list(self, item):
        func = 'GetGlobalOptionCodelist(%s)'%item
        codes = self.dynamicCall(func)
        return codes.split(';')
    ...
    def get_comm_data(self, tr_code, multi_data_name):
        """
        멀티데이터 획득 메서드
        ...
        data = self.dynamicCall("GetCommData(QString, QString)", tr_code, multi_data_name)
        return data
