...
def merge_data(df_cur_sel, df_bid_sel, df_mem_sel, code, month):
    begin_time = df_bid_sel.loc[0,'시간']
    cur_time = datetime.strptime(begin_time, '%Y-%m-%d %H:%M:%S')
    
    if os.path.exists('../Data/hdf_2017%02d/%s.h5'%(month, code)):
        try:
            old_df = pd.read_hdf('../Data/hdf_2017%02d/%s.h5'%(month, code), 'table').reset_index()
        except KeyError:
    ...
    while True:
        ...
        next_time = datetime.strptime(next_time, '%Y-%m-%d %H:%M:%S')
        cur_price = df_cur_sel.iloc[idx[0],4]
        hoga_sell_1 = df_bid_sel.iloc[idx[1],23]
        hoga_buy_1 = df_bid_sel.iloc[idx[1],25]
        try:
            total_cnt[cur_price] += df_cur_sel.loc[idx[0], '체결량']
        except KeyError:
            total_cnt[cur_price] = df_cur_sel.loc[idx[0], '체결량']
        print("time:%s, cur:%d, sell[1]:%d, buy[1]:%d, ncur:%d, nbid:%d" % (cur_time, cur_price, hoga_sell_1, hoga_buy_1, df_cur_sel.loc[idx[0], '체결량'], df_bid_sel.iloc[idx[1],4]))
    ...
    new_df.to_hdf('../Data/hdf_2017%02d/%s.h5'%(month, code), 'table', append=True)
...
def main():
    month = 9
    if os.path.exists('../Data/hdf_2017%02d/A005930.h5'%month):
        tmp_df = pd.read_hdf('../Data/hdf_2017%02d/A005930.h5'%month, 'table').reset_index()
        saved_time = tmp_df.loc[len(tmp_df)-1, '시간'][:10]
    else:
        if not os.path.exists('../Data/hdf_2017%02d'%month):
            os.makedirs('../Data/hdf_2017%02d'%month)
        saved_time = "2000-00-00"
    flist = glob.glob('../StockCur/2017-%02d*.csv'%month)
    for afile in sorted(flist[:1]):
        fname = afile[-14:]
        new_time = fname[:10]
        ...
        q = Queue()
        p = []
        for code in tqdm(code_list[10:11], ncols=80):
            code = 'A005930'
            ...
            p.append(Process(target=merge_data, args=(df_cur_sel, df_bid_sel, df_mem_sel, code, month)))
            p[-1].start()
        for pr in p:
            pr.join()
